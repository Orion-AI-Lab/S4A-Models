...
if __name__ == '__main__':
    ...
    parser.add_argument('--checkpoints', nargs='+', required=True,
                            help='The checkpoints to load for every model. First the ConvLSTM, second the ConvSTAR, third the UNet.')
    ...
    unet = UNet(run_path_unet, LINEAR_ENCODER, parcel_loss=args.parcel_loss, num_layers=3)

    # Load the model for testing
    checkpoint_epoch = Path(unet_chkpt).stem.split('=')[1].split('-')[0]
    unet = UNet.load_from_checkpoint(unet_chkpt,
                                     map_location=torch.device('cpu'),
                                     run_path=run_path_unet,
                                     linear_encoder=LINEAR_ENCODER,
                                     checkpoint_epoch=checkpoint_epoch,
                                     num_layers=3)
    ...
    for image_id in image_idx:
        ...
        grid1 = ImageGrid(fig, 141, nrows_ncols=(num_subpatches[0], num_subpatches[1]), axes_pad=0.0)
        grid2 = ImageGrid(fig, 142, nrows_ncols=(num_subpatches[0], num_subpatches[1]), axes_pad=0.0)
        grid3 = ImageGrid(fig, 143, nrows_ncols=(num_subpatches[0], num_subpatches[1]), axes_pad=0.0)
        grid4 = ImageGrid(fig, 144, nrows_ncols=(num_subpatches[0], num_subpatches[1]), axes_pad=0.0)
        for idx in range(num_subpatches[0] * num_subpatches[1]):
            ...
            pred_sparse_unet = pred_unet.argmax(axis=1).squeeze().cpu().detach().numpy()

            grid2[idx].imshow(pred_sparse_unet, vmin=0, vmax=max(LINEAR_ENCODER.values()), cmap='tab20')
            grid2[idx].set_axis_off()

            grid3[idx].imshow(pred_sparse_convlstm, vmin=0, vmax=max(LINEAR_ENCODER.values()), cmap='tab20')
            grid3[idx].set_axis_off()

            grid4[idx].imshow(pred_sparse_convstar, vmin=0, vmax=max(LINEAR_ENCODER.values()), cmap='tab20')
            grid4[idx].set_axis_off()
        ...
        axes[3].legend(handles=patches, bbox_to_anchor=(1.05, 1), loc=2, borderaxespad=0., fontsize='x-large')

        axes[0].set_title('Label', fontdict=title_font)
        axes[1].set_title('U-Net Prediction', fontdict=title_font)
        axes[2].set_title('ConvLSTM Prediction', fontdict=title_font)
        axes[3].set_title('ConvSTAR Prediction', fontdict=title_font)
        ...
        plt.savefig(out_path / f'exp{args.exp}_comparative_evaluation_of_image_{image_id}_PREDS_with_unet.png', dpi=fig.dpi, bbox_inches='tight', pad_inches=0.5)
